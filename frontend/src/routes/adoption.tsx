...
export const Route = createFileRoute("/adoption")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Browse cats waiting for a forever home. Filter by personality, age, and more.",
      },
    ],
  }),
  component: Adoption,
...
const tags = ["All", "Kittens", "Adult", "Senior", "Indoor", "Good with kids", "Lap cats", "Adventurous"];

function Adoption() {
  const cats = generateCats(12, "adoption");
  ...
  return (
    <>
      <PageHero
        ...
        title={
          <>
            Soft hearts looking for <span className="text-gradient">a forever home.</span>
          </>
        }
          ...
          {tags.map((t, i) => (
            <button
              ...
              className={`rounded-full px-4 py-1.5 text-xs font-semibold shadow-soft transition ${
                i === 0 ? "gradient-primary text-primary-foreground" : "bg-white text-foreground hover:bg-white/80"
              }`}
            >
              {t}
            </button>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24">
        ...
        <div className="mt-16 flex justify-center">
          <button className="btn-bounce inline-flex items-center gap-2 rounded-full gradient-primary px-7 py-3 text-sm font-bold text-primary-foreground shadow-soft">
            <Heart className="h-4 w-4" /> Load more sweet faces
          </button>
        </div>
      </section>
    </>
